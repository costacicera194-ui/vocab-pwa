    ...
    const response = await fetch(`https://api.deepseek.com/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `你是一个最严格的考研英语真题数据库。你的唯一任务是：从历年中国研究生入学考试（考研）英语一或英语二的真实试卷原文中，精确检索出包含特定单词的“真实考试原句”，并提供中文翻译。\n【三大铁律】：\n1. 英文原句必须 100% 来自历年考研真题，绝不可编造！\n2. 该单词在句中的词义必须贴近用户提供的中文释义！\n3. 必须严格按照以下格式输出：\n英文原句\n---\n中文翻译\n\n【附加要求】：在“中文翻译”部分，请务必用两个星号将该单词对应的中文词义包裹起来（例如：人类对于语言的**能力**是惊人的）。如果实在找不到匹配词义的真题，请回复：(未找到匹配该词义的考研真题)---(无翻译)`
          },
          {
            role: "user",
            content: `单词：${word}\n限定释义：${meaning}`
